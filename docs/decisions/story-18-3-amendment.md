...
> **RETIRED POST-STORY-27 (W4, 2026-09-08) — the cap became moot because the SIGNAL was retired, not because the reasoning changed.** The reasoning above is correct and is preserved for whoever revisits Certificate Transparency. What changed is availability: first-cert age needs certificate *history*, and history is unobtainable — crt.sh has been down the entire build (5xx → 502 → 403, re-confirmed **HTTP 502 on 2026-09-08**), SSLMate Cert Spotter returns **unexpired certs only** (verified live: the earliest issuance it returns for 31-year-old stripe.com is ~4 months old), and no other free CT-history source exists. The signal was already demoted to corroborating (Stage 3a), capped, and had **never once run against a real certificate** (0 of 49 corpus domains — §5.3). Keeping a signal we cannot feed is dishonest, so it was removed: `CT_INTERPRETABLE_FROM_ISO`, `CERT_AGE_CAP_YEARS`, `certAgeClaim`, `certAgeIsFloorOnly`, the corroboration branch, and the `first_cert_date`/`cert_count` signals are gone. `certs.ts` now reports only the **current** certificate (live TLS handshake, with SSLMate as an availability fallback for issuer + validity), published as neutral facts that feed no verdict. **If crt.sh ever returns**, the cap logic here is the record to restore first-cert age from — the min()-lower-bound rule and the April-2018 boundary still hold. (Owner ruling: retire, with the indicator exception granted, 2026-09-08.)
