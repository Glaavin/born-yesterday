...
export default function RecentSearches({
  ...
  return (
    <section className="w-full max-w-lg" aria-labelledby="recent-heading">
      <h2 id="recent-heading" className="mb-2 text-center text-sm text-label-teal">
        Recently checked
      </h2>
      <ul className="flex flex-col gap-1.5">
        {items.map((r) => (
          <li
            ...
            className="flex items-center justify-between gap-3 rounded-md border border-ink-muted/20 px-3 py-2 text-sm"
          >
            <Link href={`/r/${encodeURIComponent(r.domain)}`} className="truncate text-ink hover:text-accent-gold">
              {r.domain}
            </Link>
            <span
              className={`shrink-0 rounded border px-1.5 py-0.5 text-xs font-medium ${REPORT_STATES[r.state].pill}`}
            >
